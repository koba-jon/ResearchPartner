...
The GETTING_STARTED manual and Auto mode are generated by default; the experiment/record labels default to Experiment / Note. All of these are configurable afterwards in `researchpartner.config.json`. `ENABLE_PUBLISH_STEP` is **on** by default: generated Claude Code prompts end with a Publish step that commits and pushes the docs repo *after the task's verification passes* (it assumes you completed the one-time `origin` setup in step 4). Set it to `no` to keep commit/push manual.
...
`make docs-check` (the pre-commit hook runs it too) verifies: required files exist, markdown links and backtick doc-paths resolve, section references resolve, routers reach their bodies, and the two byte-identical mirror blocks match. Docs are language-agnostic (Unicode) — any script and math notation (Greek letters, symbols) is allowed. If you had hand-edited a framework file, `make update` never overwrites it: it writes the new version alongside as `<file>.rp-new` and warns, so you can compare, merge, and delete the `.rp-new`.
...
- **Docs came out in English even though I set another docs language** — the skeleton always ships in English; `init` does not translate it (the engine is dependency-free). With a non-English `DOCS_LANG`, your rendered `docs/entrypoint.md` carries a "First-session setup" block, and on the first session your Research Partner offers a one-time localization via a Claude Code prompt (`docs/operations/prompt-factory.md` section 4.6); new docs are authored in your language thereafter. There is no character/script policy, so Greek math letters and any non-Latin script are always allowed.
- **Adopting this onto a project that already has results** — on the first session your Research Partner offers to bootstrap the docs from your existing artifacts (prior results, logs, notebooks, writeups) via a read-only Claude Code ingest (`docs/operations/prompt-factory.md` section 4.7); it asks first and seeds `comparison.md` / `project-status.md` / the theme index from real state. The shipped `(example)` rows are placeholders until then.
...
Edits to `templates/` and `scripts/` are linted on commit by `make lint-base` (token sanity, single-brace `${TOKEN}` leak detection, manifest coverage, fragment presence, and a render of every template to catch unbalanced conditionals). Run it before pushing the base.

```bash
make test        # stdlib-only suite: python3 -m unittest discover -s tests -p 'test_*.py'
```

The suite covers the engine (tokens, conditionals, includes), the config/derivation layer, the ownership manifest, base-side lint (`lint_base`, including single-brace token-leak detection), the consistency guard (each defect class fails: mirror drift, dangling backtick path, missing required file, broken router; titled links and non-English/Unicode docs pass), and end-to-end lifecycle round-trips (`init`, `update` `.rp-new` safety, `sync_src`, base-protection, the leanest/all-on render matrix). CI (`ci.example.yml`) runs `lint-base` + the suite across Python 3.8–3.12.
