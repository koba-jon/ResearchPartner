...
- **Detect & ingest** — a read-only scan of your workspace. If it finds a `src/` directory, your code is mirrored (read-only) into the clone's `src/` so the Claude Project seat can read it; if no code is found, it asks a few intent questions instead.
...
Copy `researchpartner.config.example.json` as a starting point. `--dry-run` shows the planned writes without touching disk; `--print-config` prints the fully-resolved config.
