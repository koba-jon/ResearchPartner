...
- **Docs came out in English even though I set another docs language** — the skeleton always ships in English; `init` does not translate it (the engine is dependency-free). With a non-English `DOCS_LANG`, your rendered `docs/entrypoint.md` carries a "First-session setup" block, and on the first session your Research Partner offers a one-time localization via a Claude Code prompt (`docs/operations/prompt-factory.md` section 4.7); new docs are authored in your language thereafter. There is no character/script policy, so Greek math letters and any non-Latin script are always allowed.
- **Adopting this onto a project that already has results** — on the first session your Research Partner offers to bootstrap the docs from your existing artifacts (prior results, logs, notebooks, writeups) via a read-only Claude Code ingest (`docs/operations/prompt-factory.md` section 4.6); it asks first and seeds `comparison.md` / `project-status.md` / the theme index from real state. The shipped `(example)` rows are placeholders until then.
